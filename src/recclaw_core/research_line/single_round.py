...
_ROUND_TEST_FEEDBACK_MANIFEST_SCHEMA = (
    "recclaw.round-test-feedback-partition-manifest.v1"
)
_ROUND_TEST_FEEDBACK_MANIFEST_IDENTITY = {
    "dataset": "ml-1m",
...
_ROUND_TEST_FEEDBACK_FILES = {
    "train": "ml-1m.train.inter",
    "dev": "ml-1m.dev.inter",
...
def validated_search_partition_identity() -> dict[str, Any]:
    """Validate the selected formal round-test-feedback data before launch."""
    ...
    try:
        ...
        raise ValueError(
            f"selected search partition manifest is unreadable: {manifest_path}"
        ) from error
    if not isinstance(manifest, Mapping):
        raise ValueError("selected search partition manifest must be a JSON object")
    if manifest.get("schema") != _ROUND_TEST_FEEDBACK_MANIFEST_SCHEMA:
        raise ValueError(
            "selected search partition manifest is not the formal "
            "round-test-feedback manifest"
        )
    mismatches = {
        name: manifest.get(name)
        for name, expected in _ROUND_TEST_FEEDBACK_MANIFEST_IDENTITY.items()
        if manifest.get(name) != expected
    }
    if mismatches:
        raise ValueError(
            "selected search partition manifest has the wrong round-test-feedback "
            f"identity: {sorted(mismatches)}"
        )

    files = manifest.get("files")
    required_filenames = set(_ROUND_TEST_FEEDBACK_FILES.values())
    if not isinstance(files, Mapping) or set(files) != required_filenames:
        raise ValueError(
            "selected search partition manifest must declare exactly "
            "ml-1m.train.inter, ml-1m.dev.inter, ml-1m.heldout.inter, "
            "ml-1m.user, and ml-1m.item files"
        )
    ...
    return canonical_value(
        {
            "manifest_sha256": _bytes_sha256(manifest_path),
    ...
    return ResearchContext(
        ...
        knowledge_base={
            ...
            "open_capability_escape_hatch": True,
                "protocol": (
                    "ML-1M train/dev/heldout full-sort with per-round "
                    "best-checkpoint test feedback"
                ),
        },
        frozen_goal={
            "metric": "NDCG@10",
    ...
    manifest = canonical_value(
        {
            ...
            "experiment": {
                ...
                "split": canonical_value(COMMON_SPLIT),
            ...
            "claim_boundary": {
                ...
                "test_feedback_scope": "PER_ROUND_EXECUTION",
                "test_feedback_selection": (
                    "BEST_CHECKPOINT_SELECTED_ON_DEV"
                ),
...
class _RecordedProducer:
    ...
    def __init__(self, provider: ProviderResearchProducer) -> None:
        ...
        self._records: dict[str, Mapping[str, Any] | Exception] = {}

    @property
    def call_traces(self) -> tuple[Mapping[str, Any], ...]:
        return self.provider.call_traces

    def __call__(self, role: str, view: Mapping[str, Any]) -> Mapping[str, Any]:
        record = self._records.get(role)
        if record is None:
            try:
                record = canonical_value(dict(self.provider(role, view)))
    ...
    *,
    ...
    try:
        result = run_research_round(
            ...
            carryover_proposals=bootstrap_search_pool(
                composition.context,
                composition.profile,
                composition.policy,
            ),
