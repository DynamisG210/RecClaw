...
def _resource_view(
    ...
    *,
    ...
    if upper > budget_seconds:
        ...
        training_lower_bound_seconds = _finite(
            prediction.get("training_only_lower_bound_seconds"),
            name="resource_profile.prediction.training_only_lower_bound_seconds",
            low=0.0,
        )
        if (
            worker_ceiling_seconds > budget_seconds
            or point_seconds > budget_seconds
            or training_lower_bound_seconds > budget_seconds
        ):
            _fail("resource profile prediction interval exceeds its finite budget")
    peak_memory = _finite(
