...
def _eligibility(
    ...
    *,
    ...
    # An unknown or not-yet-validated parent is pre-outcome uncertainty, not
    # proof that the candidate cannot be trained.  It is already reflected in
    # ``effective_lineage_risk`` and therefore lowers the portfolio score.
    # Hard-block only a parent that is known to have failed; otherwise a fresh
    # campaign would be unable to execute its first qualified child.
