"""Deterministic per-round Search materialization from the active profile."""
...
def _active_catalog(
    ...
    for entry in profile.entries:
        ...
        if mechanism is None:
            ...
            if origin == "FIXED_66":
                raise ValueError(
                    "fixed Search profile entry is not an exact BL-ICF "
                    "catalog mechanism"
                )
            # Qualified Innovation entries are supplied by the durable
            # carryover catalog; the deterministic refill covers the static
            # Fixed66 remainder without pretending they are built-in entries.
            continue
...
def _select_catalog_entries(
    ...
    *,
    ...
    max_proposals: int,
    preferred_semantics: Sequence[str] = (),
    rotation: int = 0,
) -> tuple[tuple[SearchExecutableEntryV1, ExecutableMechanismV1], ...]:
    by_axis: dict[
    ...
    for semantics_digest in preferred_semantics:
        item = next(
            (
                candidate
                for candidate in active
                if candidate[0].semantic_identity_digest == semantics_digest
            ),
            None,
        )
        if item is None or item[1].mechanism_id in selected_ids:
            continue
    ...
    if selected and max_proposals > 1 and not any(
        ...
        if fallback is not None:
            if len(selected) < max_proposals:
                ...
                selected[-1] = fallback
            selected_ids = {item[1].mechanism_id for item in selected}

    if len(selected) >= max_proposals:
        return tuple(selected[:max_proposals])

    # Axis representatives are the high-value prefix.  A formal round needs a
    # deep enough deterministic bench to recover from candidate-local failures,
    # so fill the remaining bounded slots from the complete active catalog.
    remaining = sorted(
        (
            item
            for item in active
            if item[1].mechanism_id not in selected_ids
        ),
        key=_candidate_sort_key,
    )
    if remaining:
        offset = rotation % len(remaining)
...
def _pending_task_semantics(context: ResearchContext) -> tuple[str, ...]:
    ...
    task = ResearchTaskQueueV2.from_dict(raw_queue).select_next()
    return (task.candidate_semantic_digest,) if task is not None else ()
...
def bootstrap_search_pool(
    ...
    *,
    max_proposals: int = 4,
    excluded_mechanism_ids: Sequence[str] = (),
) -> tuple[CandidateProposalV4, ...]:
    """Build deterministic typed proposals for the current ordered round."""
    ...
    if any(
        ...
        for item in excluded_mechanism_ids
    ):
        raise ValueError("excluded_mechanism_ids must be normalized strings")
    if (
    ...
    all_active = _active_catalog(active_profile)
    excluded = set(excluded_mechanism_ids)
    active = tuple(
        item for item in all_active if item[1].mechanism_id not in excluded
    )
    if not active:
        return ()
    ...
    selected = _select_catalog_entries(
        ...
        preferred_semantics=_pending_task_semantics(context),
        rotation=(context.round_index - 1) * max_proposals,
    )
    if not selected:
        return ()

    active_by_id = {
        mechanism.mechanism_id: mechanism for _, mechanism in all_active
    }
    ...
    for index, (entry, mechanism) in enumerate(selected):
        ...
        proposals.append(
            CandidateProposalV4(
                ...
                candidate_label=(
                    f"Round {context.round_index} materialized: {mechanism.mechanism_id}"
                ),
                mechanism_hypothesis=hypothesis,
