...
def execution_recipe_for_search_binding(
    ...
    *,
    ...
    if entry.origin is SearchProfileEntryOriginV1.FIXED_66:
        ...
        # Native observation carries protocol cadence beside the qualified
        # source inputs. Runtime binds it after adapter attestation, exactly
        # where the disposable resource profile formerly supplied cadence.
        extra = sorted(supplied_fields - _QUALIFIED_EXECUTION_FIELDS - {"native_training_cadence"})
        if missing or extra:
            detail = []
            if missing:
        ...
        projected.pop("native_training_cadence", None)
...
def project_common_execution_feedback(
    *,
    ...
    if selected_outcome.source_proposal is not None:
        if (
            selected_outcome.source_proposal.candidate_id
            != binding.proposal.candidate_id
        ):
            raise ValueError("selected Search outcome differs from its binding")
    elif is_qualified_open_spec_candidate(binding.proposal):
    ...
    try:
        ...
        raise ValueError(str(error)) from error
    normalized_evaluator = canonical_value(
        dict(_require_mapping(evaluator, field_name="evaluator"))
    )
    if metric_contract_digest != sha256_digest(normalized_evaluator):
        raise ValueError(
            "metric_contract_digest is not the supplied evaluator identity"
        )
    ...
    event = SearchUtilityEventV2(
        ...
        candidate_value=candidate_metric if status == "SUCCESS" else None,
