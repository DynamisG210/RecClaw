...
DEVELOPMENT_SPLIT = "train/dev/dev"
DEVELOPMENT_EVALUATOR = canonical_value(
    {
        "candidate_universe": "FULL_SORT",
...
P4_SPARSE_SPECTRAL_SPLIT = "p4-ml1m-rs811-validation"

P4_SPARSE_SPECTRAL_EVALUATOR = canonical_value(
    {
        "candidate_universe": "FULL_SORT",
        "heldout_access": "POST_VALIDATION_SELECTION_ONLY",
        "metric": "NDCG@10",
        "nonfinite_policy": "REJECT",
        "partition_role": "P4_VALIDATION_SELECTION",
        "protocol_family": "P4_SPARSE_SPECTRAL",
        "source": "FROZEN_RECBole_RS811_VALIDATION_RESULT",
        "test_mask": "TRAIN_PLUS_VALIDATION_ONLY_FOR_FINAL_CONFIRMATION",
    }
)

_SUPPORTED_EVALUATION_CONTRACTS = (
    (P4_SPARSE_SPECTRAL_SPLIT, P4_SPARSE_SPECTRAL_EVALUATOR),
    (COMMON_SPLIT, COMMON_EVALUATOR),
    (DEVELOPMENT_SPLIT, DEVELOPMENT_EVALUATOR),
)
...
def validate_execution_recipe(recipe: Mapping[str, Any]) -> None:
    ...
    contract_dataset = config.get("dataset", COMMON_DATASET)
    if (
        not isinstance(contract_dataset, str)
        or not contract_dataset
        or contract_dataset != contract_dataset.strip()
    ):
        raise ExperimentBindingError(
            "execution_recipe config dataset must be normalized and non-empty"
        )
    if recipe["dataset"] != contract_dataset:
        raise ExperimentBindingError(
            "execution_recipe dataset must match the execution contract dataset "
            f"{contract_dataset!r}"
        )
    ...
    if not any(
        recipe["split"] == expected_split
        and normalized_evaluator == expected_evaluator
        for expected_split, expected_evaluator in _SUPPORTED_EVALUATION_CONTRACTS
    ):
        raise ExperimentBindingError(
            "execution_recipe split/evaluator must be one exact supported "
            "FULL_SORT NDCG@10 contract"
        )
    ...
    source_content_digest = recipe.get("candidate_source_content_digest")
    if source_content_digest is not None and (
        not isinstance(source_content_digest, str) or len(source_content_digest) != 64
    ):
        raise ExperimentBindingError("candidate_source_content_digest is invalid")
...
class ExperimentBindingV1:
    ...
    candidate_source_content_digest: str | None
    entrypoint: str
    ...
    def from_execution_recipe(
        ...
        *,
        ...
        return cls(
            ...
            candidate_source_content_digest=recipe.get(
        ...
        return canonical_value(
            {
                ...
                "candidate_source_content_digest": (
                    self.candidate_source_content_digest
                ),
