"""Fresh post-M6I V23 Pilot on the qualified gpu35 backend closure."""
...
V23_PILOT_SEARCH_SEED = 9225
V23_PILOT_ROUNDS_PER_ARM = 5
V23_PILOT_EXPERIMENT_ID = (
    "HELIX-ABC-DEVELOPMENT-CAMPAIGN-PILOT-9225-V23"
)
V23_PILOT_ASSIGNMENT_NONCE = "M6I-V23-PILOT-9225-OPAQUE-V1"
V23_EXECUTABLE_PROFILE_DIGEST = V16_EXECUTABLE_PROFILE_DIGEST
V23_RESOURCE_ENVELOPE_RESOURCE = (
    "pilot_v23_gpu35_resource_envelope.json"
)


def v23_arm_policies() -> tuple[ArmPolicyV1, ArmPolicyV1, ArmPolicyV1]:
    """Return the exact frozen V16 A/B/C treatment policies."""

    return v16_arm_policies()


def v23_resource_ceilings() -> ResourceCeilingsV1:
    payload = json.loads(
        resources.files(
            "recclaw_core.experiments.helix_abc_v1.resources"
        )
        .joinpath(V23_RESOURCE_ENVELOPE_RESOURCE)
        .read_bytes()
    )
...
class V23PilotStoreContractV1:
    ...
    def create(cls) -> "V23PilotStoreContractV1":
        ...
        payload = {
            ...
            "experiment_id": V23_PILOT_EXPERIMENT_ID,
            "formal_acceptance": False,
            "main_eligibility": False,
            "ordinary_execution_seed": base.ordinary_execution_seed,
            "scheduled_slots_per_arm_seed": V23_PILOT_ROUNDS_PER_ARM,
            "search_seeds": [V23_PILOT_SEARCH_SEED],
        }
        return cls(
            experiment_id=V23_PILOT_EXPERIMENT_ID,
...
def v23_pilot_guard_context() -> GuardContext:
    ...
    return GuardContext(
        claim={
            **claim,
            "claim_id": "CLAIM-M6I-V23-PILOT-9225",
        ...
        current_evidence={
            **evidence,
            "snapshot_id": "M6I-V23-PILOT-9225-EMPTY",
            "claim_id": "CLAIM-M6I-V23-PILOT-9225",
            "protocol_id": protocol["protocol_id"],
        },
    )
...
class V23PilotOrchestratorV1(MetaV17PilotOrchestratorV1):
    def __init__(
        self,
        root: Path,
        *,
        ...
        if (
            ...
            raise PreCanaryInvariantError(
                "V23 Pilot requires the direct pinned Main Original path"
            )
        if broker.campaign_meta_runtime is not meta_runtime:
            raise PreCanaryInvariantError(
                "V23 Broker and orchestrator require the same V19 runtime"
            )
        expected_ceilings = v23_resource_ceilings()
        super().__init__(
            root,
            broker=broker,
            ...
            _contract=V23PilotStoreContractV1.create(),
            _assignment_nonce=V23_PILOT_ASSIGNMENT_NONCE,
            _guard_context=v23_pilot_guard_context(),
        ...
        if self.resource_ceilings != expected_ceilings:
            raise PreCanaryInvariantError(
                "V23 runtime resource envelope binding mismatch"
            )
    ...
    def run_round_triplet(
        ...
        *,
        ...
        return super().run_fake_triplet(
            search_seed=V23_PILOT_SEARCH_SEED,
    ...
    def run_pilot(
        ...
        return tuple(
            ...
            for round_index in range(1, V23_PILOT_ROUNDS_PER_ARM + 1)
        )
...
__all__ = [
    ...
    "V23_PILOT_EXPERIMENT_ID",
    "V23_PILOT_ROUNDS_PER_ARM",
    "V23_PILOT_SEARCH_SEED",
    "V23PilotOrchestratorV1",
    "V23PilotStoreContractV1",
    "v23_arm_policies",
    "v23_pilot_guard_context",
