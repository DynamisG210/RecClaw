#!/usr/bin/env python3
"""Launch one measured V22 gpu35 qualification canary."""
...
def main() -> int:
    ...
    canary_parent = backend / "backend_canaries_v22"
    ...
    command = [
        str(python),
        str(source / "scripts/run_v22_full_recipe_canary.py"),
        "--output-root",
        str(output_root),
        "--python",
        str(python),
        ...
        args.mechanism_id,
        "--experiment-id",
        f"V22-GPU35-FULL-RECIPE-CANARY-{args.search_seed}",
        "--lineage-partition",
        "V22_GPU35_QUALIFICATION_EXCLUDED_FROM_PILOT_AND_MAIN",
        "--result-filename",
        result_filename,
        "--search-seed",
