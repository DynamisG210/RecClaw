#!/usr/bin/env python3
"""Launch one measured V23 gpu35 qualification canary."""
...
def main() -> int:
    ...
    canary_parent = backend / "backend_canaries_v23"
    ...
    result_filename = (
        f"GPU35_V15_{args.name.upper()}_{args.search_seed}_RESULT.json"
    )
    command = [
        str(python),
        str(source / "scripts/run_v23_full_recipe_canary.py"),
        "--output-root",
        str(output_root),
        "--python",
        str(python),
        ...
        args.mechanism_id,
        "--experiment-id",
        f"V23-GPU35-FULL-RECIPE-CANARY-{args.search_seed}",
        "--lineage-partition",
        "V23_GPU35_QUALIFICATION_EXCLUDED_FROM_PILOT_AND_MAIN",
        "--result-filename",
        result_filename,
        "--search-seed",
