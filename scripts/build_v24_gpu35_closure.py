#!/usr/bin/env python3
"""Build and verify the complete pre-outcome V24 gpu35 Pilot closure."""
...
MARGIN_POLICY = DOCS / "V24_GPU35_RESOURCE_MARGIN_POLICY_V1.json"
RESOURCE_ENVELOPE = RESOURCES / "pilot_v24_gpu35_resource_envelope.json"
SOURCE_PROJECTION = (
    DOCS / "V24_V16_SCIENTIFIC_SOURCE_PROJECTION.json"
)
RUNTIME_AUDIT = DOCS / "V24_GPU35_RUNTIME_AUDIT.json"
BACKEND_AUDIT = (
    DOCS / "V24_GPU35_BACKEND_CONFORMANCE_AUDIT.json"
)
SCIENTIFIC_GATE = DOCS / "V24_SCIENTIFIC_ATTRIBUTION_GATE.json"
ORIGINAL_PROBE = DOCS / "V24_PINNED_ORIGINAL_MATERIALIZATION_PROBE.json"
DEFAULT_OUTPUT = DOCS / "V24_FROZEN_CHAIN_PILOT_CONTRACT.json"
DEFAULT_OUTPUT_ROOT = BACKEND_ROOT.parent / "pilot_9226_v24"
...
TRAINING_RELEASE = RESOURCES / "training_runtime_release_v16.json"
TRAINING_LOCK = RESOURCES / "training_runtime_v16_lock.json"
BROKER_RELEASE = (
...
RECORD_SCHEMA = "recclaw.v24-pilot-contract.v1"
RUNTIME_RELEASE_ID = "TRAINING_RUNTIME_RELEASE_V16"
CANARY_SPECS = (
    (
        "bpr",
        "BPR_MF",
        9380,
        "GPU35_V16_BPR_9380_RESULT.json",
    ),
    (
        "lightgcn",
        "LIGHTGCN",
        9381,
        "GPU35_V16_LIGHTGCN_9381_RESULT.json",
    ),
    (
        "compositional",
        "LIGHTGCN__LGCN_AUX_ALIGNMENT__LGCN_DUAL_PATH",
        9382,
        "GPU35_V16_COMPOSITIONAL_9382_RESULT.json",
    ),
)
V24_OVERLAY_FILES = (
    "docs/research_line/continuous_program/"
    "V24_GPU35_RESOURCE_MARGIN_POLICY_V1.json",
    "docs/research_line/continuous_program/"
    "V24_PINNED_ORIGINAL_MATERIALIZATION_PROBE.json",
    "scripts/build_v24_gpu35_closure.py",
    "scripts/freeze_campaign_training_runtime_release_v16.py",
    "scripts/freeze_lab_api_broker_release_v24.py",
    "scripts/launch_v24_qualification_canary.py",
    "scripts/launch_v24_gpu35_supervised.ps1",
    "scripts/run_v24_full_recipe_canary.py",
    "scripts/run_v24_original_materialization_probe.py",
    "scripts/run_v24_pilot.py",
    "scripts/validate_campaign_training_runtime_v16.py",
    ...
    "src/recclaw_core/experiments/helix_abc_v1/resources/"
    "lab_api_broker_release_v1_v24_schema_v6.json",
    "src/recclaw_core/experiments/helix_abc_v1/resources/"
    "pilot_v24_gpu35_resource_envelope.json",
    "src/recclaw_core/experiments/helix_abc_v1/resources/"
    "training_runtime_release_v16.json",
    "src/recclaw_core/experiments/helix_abc_v1/resources/"
    "training_runtime_v16_lock.json",
    "tests/experiments/helix_abc_v1/test_v24_prequalification.py",
)
...
def _broker_release() -> dict[str, Any]:
    ...
    schema_digest = file_sha256(PROPOSAL_SCHEMA)
    if (
        ...
        or release.get("response_schema_digest") != schema_digest
    ):
        raise RuntimeError("V24 Provider transport release is invalid")
    return release
...
def _canary_records() -> list[dict[str, Any]]:
    parent = BACKEND_ROOT / "backend_canaries_v24"
    records = []
...
def _build_resource_envelope(
    ...
    envelope = {
        ...
        "record_schema": "recclaw.v24-resource-envelope.v1",
...
def _source_projection(
    ...
    for relative, digest in m6i_projection.items():
        if relative not in post_m6i_runtime_files:
            if file_sha256(ROOT / relative) != digest:
                raise RuntimeError(
                    f"M6I qualified source changed before V24 freeze: {relative}"
                )
    ...
    overlay = {
        relative: file_sha256(ROOT / relative)
        for relative in V24_OVERLAY_FILES
    }
    projection = {
        "allowed_v16_byte_difference": {
            runtime_binding: (
                "active Campaign runtime resource binding V9 -> V16"
            ),
            ...
            "V24_PROVIDER_TRANSPORT_SCHEMA": (
                "the Provider response parent field is transport-enforced "
                "null; exact same-Arm lineage is bound by the canonical "
                "runtime without changing the frozen 66-semantics profile"
            ),
        },
        "authority": "NONE",
        "evidence_class": "DEVELOPMENT_ONLY_PRE_OUTCOME",
        ...
        "record_schema": (
            "recclaw.v24-v16-scientific-source-projection.v1"
        ),
...
def _runtime_audit(
    ...
    if (
        ...
        raise RuntimeError(
            f"Runtime V16 validation failed: {tuple(failures)}"
        )
    audit = {
        "authority": "NONE",
        ...
        "evidence_class": "DEVELOPMENT_ONLY_PRE_OUTCOME",
        "formal_acceptance": False,
        "record_schema": "recclaw.v24-gpu35-runtime-audit.v1",
...
def _backend_audit(
    ...
    audit = {
        ...
        "record_schema": (
            "recclaw.v24-gpu35-backend-conformance-audit.v1"
        ),
...
def _scientific_gate(
    ...
    gate = {
        ...
        "checked_invariants": [
            ...
            "provider_transport_parent_is_null_and_runtime_owned",
        ...
        "pilot_outcomes_used": False,
        "provider_calls": 0,
        "record_schema": "recclaw.v24-scientific-attribution-gate.v1",
        "runtime_audit_digest": runtime["runtime_audit_digest"],
        "scientific_source_projection_digest": source_projection[
            "projection_digest"
        ],
...
def _build_contract(
    *,
    ...
    store = V24PilotStoreContractV1.create()
    if (
        tuple(item.to_dict() for item in v24_arm_policies())
        != tuple(item.to_dict() for item in v16_arm_policies())
        or v24_resource_ceilings().to_dict()
        != envelope["resource_ceilings"]
    ):
        raise RuntimeError("V24 treatment/resource binding mismatch")
    assignment = PrivateTreatmentAssignmentV1.create(
        store.experiment_id,
        nonce=V24_PILOT_ASSIGNMENT_NONCE,
    )
    ...
    broker_release = _broker_release()
    payload["broker"] = {
        **v16["broker"],
        "endpoint_digest": broker_release["endpoint_digest"],
        "max_total_tokens_per_call": broker_release[
            "max_total_tokens_per_call"
        ],
        "model": broker_release["model"],
        "release_digest": broker_release["release_digest"],
        "release_manifest_path": BROKER_RELEASE.as_posix(),
        "release_manifest_sha256": file_sha256(BROKER_RELEASE),
        "request_mode": broker_release["request_mode"],
        "response_schema_digest": broker_release[
            "response_schema_digest"
        ],
        "response_schema_path": PROPOSAL_SCHEMA.as_posix(),
        "response_schema_sha256": file_sha256(PROPOSAL_SCHEMA),
        "retry_count": broker_release["retry_count"],
        "temperature": broker_release["temperature"],
        "timeout_ms": broker_release["timeout_ms"],
        "transport": broker_release["transport"],
    }
    payload["meta"] = {
        **v16["meta"],
    ...
    payload["backend_migration"] = {
        "from_backend": "gpu35 RTX 3080 Runtime V15 (sealed V23 recoverable failure)",
        ...
        "to_backend": "gpu35 RTX 3080 Runtime V16",
    }
...
def verify_v24_pilot_contract(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or contract["pilot"]["search_seeds"] != [9226]
        ...
        or contract["broker"]["release_digest"]
        != broker_release["release_digest"]
        or contract["broker"]["response_schema_digest"]
        != file_sha256(PROPOSAL_SCHEMA)
    ):
        raise RuntimeError("V24 contract closure invariant mismatch")
    for relative, digest in contract["source"]["files"].items():
        if file_sha256(ROOT / relative) != digest:
            raise RuntimeError(f"V24 source identity mismatch: {relative}")
    exact = {
    ...
    for artifact, digest in exact.items():
        if file_sha256(artifact) != digest:
            raise RuntimeError(f"V24 release identity mismatch: {artifact}")
    if _git_head() != contract["source"]["git_head_at_freeze"]:
        raise RuntimeError("V24 source repository head is not frozen source")
    subprocess.run(
        [
            str(GIT_EXECUTABLE),
            f"--git-dir={ROOT / '.git'}",
...
def build(output: Path, output_root: Path) -> dict[str, Any]:
    if output_root.exists():
        raise RuntimeError(f"fresh V24 output root already exists: {output_root}")
    if output.exists():
        raise RuntimeError(f"V24 contract already exists: {output}")
    ...
    verify_v24_pilot_contract(output)
    return contract
...
def main() -> int:
    ...
    if args.verify:
        contract = verify_v24_pilot_contract(args.output.resolve())
    else:
        contract = build(
            args.output.resolve(),
