...
def validate_files(files: Any, proposal_type: str = "") -> list[dict[str, str]]:
    ...
    for idx, item in enumerate(files, start=1):
        ...
        validate_static_model_code(path, content, proposal_type)
        if proposal_type == "architecture":
            validate_architecture_protocol(path, content)
        normalized.append({"path": path, "content": content})
    return normalized


def validate_architecture_protocol(path: str, content: str) -> None:
    """Verify that a standalone architecture proposal implements the RecBole runner contract."""
    try:
        tree = ast.parse(content)
    except SyntaxError:
        return  # compile check already handled elsewhere

    found_class = False
    has_calculate_loss = False
    has_predict = False
        ...
        if isinstance(node, ast.ClassDef):
            ...
            for item in node.body:
                if isinstance(item, ast.FunctionDef):
                    if item.name == "calculate_loss":
                        has_calculate_loss = True
                    if item.name == "full_sort_predict":
                        has_predict = True

    if not found_class:
        raise ValueError(f"architecture proposal in {path} must define at least one class")

    if not has_calculate_loss:
        raise ValueError(
            f"architecture class in {path} must implement calculate_loss(interaction) "
            "to satisfy the RecBole runner contract"
        )

    if not has_predict:
        raise ValueError(
            f"architecture class in {path} must implement full_sort_predict(interaction) "
            "to satisfy the RecBole runner contract (task_ml1m.yaml uses mode: full)"
        )


def validate_static_model_code(path: str, content: str, proposal_type: str = "") -> None:
    try:
    ...
    is_architecture = proposal_type == "architecture"
    if (
        not is_architecture
        and SELF_REG_LOSS_PATTERN.search(content)
...
def prepare_implementation(implementation: dict[str, Any], proposal: dict[str, Any]) -> PreparedImplementation:
    ...
    files = validate_files(implementation.get("files"), proposal_type)
...
def build_implementation_context(
    ...
    is_architecture = proposal_type == "architecture"
    task_text = (
        "Implement one RecClaw architecture_required candidate proposal. "
        "The new model does NOT need to inherit BPR or LightGCN, but MUST implement the RecBole runner contract."
        if is_architecture
        else "Implement one RecClaw code_required candidate proposal under strict local allowlists."
    )
    forbidden_rules = [
    ...
    if is_architecture:
        forbidden_rules.extend([
            "Architecture proposals MUST implement calculate_loss(self, interaction) and full_sort_predict(self, interaction).",
            "Architecture proposals MAY introduce new numeric parameters in candidate_config (e.g., dropout, hidden_size, attention_heads).",
            "Architecture proposals do NOT need to inherit LightGCN or BPR; they may inherit torch.nn.Module or any RecBole abstract base, but must satisfy the runner contract.",
            "For Transformer architectures: item_embedding → positional_encoding → multi_head_self_attention → layer_norm → point_wise_feed_forward → ranking_head.",
            "For graph architectures: embedding + adjacency propagation + prediction head; keep the full-sort protocol.",
        ])
    else:
        forbidden_rules.append("Do not override full_sort_predict unless the proposal explicitly requires it.")

    action_type = str(proposal.get("action_type") or "")
    if action_type in {"loss_function", "objective_function"}:
        forbidden_rules.extend([
            "loss_function and objective_function proposals MAY override calculate_loss to replace or compose the core loss.",
            "When changing the loss, preserve the runner contract: calculate_loss must still return a scalar loss tensor.",
        ])
    ...
    context = {
        ...
        "architecture_examples": [
            "Graph models: start from embedding + adjacency propagation + prediction head (e.g., LightGCN variants, graph attention).",
            "Sequential models: start from embedding + positional encoding + self-attention blocks + FFN + prediction head (e.g., SASRec, BERT4Rec, Transformer variants).",
            "Hybrid models: combine graph propagation with a learned interaction function or sequential encoder.",
...
def main() -> int:
    ...
    try:
        ...
        if str(proposal.get("runnable_level") or "") not in {"code_required", "architecture_required"}:
