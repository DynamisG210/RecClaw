...
from recclaw_core.experiments.helix_abc_v1.prefreeze_v2 import (  # noqa: E402
    ...
    expected_prefreeze_v4_manifest,
...
def _prepare_v4() -> int:
    ...
    for forbidden in (
        ...
        if (ROOT / forbidden).exists():
            raise SystemExit("V4 outcome artifact already exists; prepare is sealed")
    if V4_PRIVATE_ROOT.exists():
        raise SystemExit("V4 private root already exists; identity is not fresh")
    _write_once(
        ROOT / V4_PROVIDER_SCHEMA_REL,
        expected_v4_provider_schema(ROOT),
    )
    _write_once(
        ROOT / V4_VALID_FIXTURE_REL,
        expected_v4_valid_fixture(ROOT),
    )
    _write_once(
        ROOT / V4_NEGATIVE_FIXTURE_REL,
        expected_v4_negative_fixture(ROOT),
    )
    _write_once(ROOT / V4_RELEASE_REL, expected_v4_release(ROOT))
    _write_once(ROOT / V4_POLICY_REL, expected_v4_retry_policy())
    _write_once(
        ROOT / V4_MANIFEST_REL,
        expected_prefreeze_v4_manifest(ROOT),
    )
    _write_once(ROOT / V4_AUTH_REL, expected_v4_authorization(ROOT))
        ...
        json.dumps(
            {
                ...
                "authorization_sha256": bytes_sha256(
                    (ROOT / V4_AUTH_REL).read_bytes()
                ),
                "manifest_sha256": bytes_sha256(
                    (ROOT / V4_MANIFEST_REL).read_bytes()
                ),
                "policy_sha256": bytes_sha256(
                    (ROOT / V4_POLICY_REL).read_bytes()
                ),
                "provider_schema_sha256": bytes_sha256(
                    (ROOT / V4_PROVIDER_SCHEMA_REL).read_bytes()
                ),
                "provider_calls": dry_run["provider_calls"],
                "request_payload_digest": manifest[
                    "exact_provider_contract"
                ]["request_payload_digest"],
                "status": "PREPARED_V4_PRE_OUTCOME",
            },
            sort_keys=True,
        )
    )
...
def _load_v4_attempt_receipt() -> tuple[dict[str, Any], dict[str, Any]]:
    manifest = validate_prefreeze_v4(ROOT)
    path = ROOT / V4_ATTEMPT_RECEIPT_REL
    if not path.is_file():
        raise SystemExit("V4 Provider attempt receipt is missing")
    receipt = json.loads(path.read_bytes())
    if canonical_json_bytes(receipt) != path.read_bytes():
        raise SystemExit("V4 Provider attempt receipt is not canonical JSON")
    fixed = {
        ...
        "local_uniqueness_contract_digest": manifest[
    ...
    for field, expected in fixed.items():
        if receipt.get(field) != expected:
            raise SystemExit(f"V4 attempt receipt does not prove {field}")
    attempts = receipt.get("physical_attempts")
    count = receipt.get("physical_provider_calls")
    if (
        not isinstance(attempts, list)
        ...
        raise SystemExit("V4 physical attempt count/retry count is invalid")
    identities = manifest["bounded_retry"]["physical_attempt_identities"]
    request_envelopes: set[str] = set()
    prior_digest: str | None = None
    prior_end_ns: int | None = None
    ...
    for index, attempt in enumerate(attempts, start=1):
        expected_identity = identities[index - 1]
        if attempt.get("ordinal") != index:
            raise SystemExit("V4 physical attempt order is not contiguous")
        if (
            attempt.get("physical_attempt_identity_digest")
            != expected_identity["physical_attempt_identity_digest"]
            or attempt.get("private_root_digest")
            ...
            raise SystemExit("V4 physical attempt identity changed")
        if (
            attempt.get("logical_call_id")
            != manifest["exact_provider_contract"]["logical_call_id"]
            or attempt.get("request_payload_digest")
            ...
            raise SystemExit("V4 diagnostic slot/payload identity changed")
        envelope = attempt.get("request_envelope_digest")
        if not isinstance(envelope, str):
            raise SystemExit("V4 request envelope digest is missing")
        request_envelopes.add(envelope)
        preimage = dict(attempt)
        attempt_digest = preimage.pop("attempt_digest", None)
        if (
            ...
            raise SystemExit("V4 physical attempt digest chain is invalid")
        prior_digest = attempt_digest
        start_ns = attempt.get("monotonic_start_ns")
        end_ns = attempt.get("monotonic_end_ns")
        if (
            ...
            raise SystemExit("V4 physical attempt time/backoff order is invalid")
        prior_end_ns = end_ns
        if (
            attempt.get("sqlite_calls_schema_digest")
            != manifest["bounded_retry"]["sqlite_calls_schema_digest"]
            ...
            raise SystemExit("V4 SQLite/provider receipt proof is invalid")
        if index < count and (
            attempt.get("retry_eligible") is not True
            or attempt.get("termination_reason") != "RETRY_SCHEDULED"
            or attempt.get("backoff_ms_after_attempt")
            ...
            raise SystemExit("V4 non-final attempt retry proof is invalid")
        if index == count and attempt.get("backoff_ms_after_attempt") != 0:
            raise SystemExit("V4 final attempt must not schedule backoff")
        required_gap_ms = int(attempt.get("backoff_ms_after_attempt", 0))
    if len(request_envelopes) != 1:
        raise SystemExit("V4 attempts did not share one request envelope")
    final = attempts[-1]
    if receipt.get("status") == "PASS":
        if (
            ...
            raise SystemExit("V4 PASS receipt is not fully closed")
    elif receipt.get("status") == "BLOCKED":
        if final.get("termination_reason") not in {
            ...
            raise SystemExit("V4 BLOCKED termination is invalid")
    else:
        raise SystemExit("V4 attempt receipt has unknown status")
    return manifest, receipt


def _dry_run_v4() -> int:
    receipt = provider_free_v4_dry_run(ROOT)
    _write_once(ROOT / V4_DRY_RUN_REL, receipt)
    print(
        json.dumps(
            {
                "dry_run_receipt_sha256": bytes_sha256(
                    (ROOT / V4_DRY_RUN_REL).read_bytes()
                ),
                "provider_calls": 0,
                "training_runs": 0,
                "status": receipt["status"],
...
def _record_v4_verification(*, pytest_passed: int, pytest_skipped: int) -> int:
    _, attempt = _load_v4_attempt_receipt()
    if attempt["status"] != "PASS":
        raise SystemExit("V4 BLOCKED attempt cannot receive PASS verification")
    dry_run_path = ROOT / V4_DRY_RUN_REL
    if not dry_run_path.is_file():
        raise SystemExit("V4 provider-free dry-run receipt is missing")
    dry_run = json.loads(dry_run_path.read_bytes())
    if (
        canonical_json_bytes(dry_run) != dry_run_path.read_bytes()
        or dry_run.get("status")
        != "PASS_PROVIDER_FREE_SEMANTIC_EQUIVALENCE"
    ):
        raise SystemExit("V4 provider-free dry-run is not PASS")
    if pytest_passed < 1 or pytest_skipped < 0:
        raise SystemExit("V4 pytest result is invalid")
    verification = {
        "schema": V4_VERIFICATION_SCHEMA,
        "status": "PASS",
        "attempt_id": V4_ATTEMPT_ID,
        "attempt_receipt_digest": bytes_sha256(
    ...
    _write_once(ROOT / V4_VERIFICATION_REL, verification)
    print(
        json.dumps(
            {
                "status": "PASS",
                "verification_receipt_sha256": bytes_sha256(
                    (ROOT / V4_VERIFICATION_REL).read_bytes()
                ),
            },
            sort_keys=True,
        )
...
def _finalize_v4() -> int:
    manifest, attempt = _load_v4_attempt_receipt()
    attempt_path = ROOT / V4_ATTEMPT_RECEIPT_REL
    if attempt["status"] == "BLOCKED":
        if (ROOT / V4_READY_REL).exists():
            raise SystemExit("V4 READY exists; refusing BLOCKED finalization")
        blocked = {
            "schema": V4_BLOCKED_SCHEMA,
            "status": "BLOCKED_PREFREEZE_V4",
            "attempt_id": V4_ATTEMPT_ID,
            "manifest_ref": V4_MANIFEST_REL.name,
            "manifest_digest": bytes_sha256(
                (ROOT / V4_MANIFEST_REL).read_bytes()
            ),
            "retry_policy_ref": V4_POLICY_REL.name,
            "retry_policy_digest": bytes_sha256(
                (ROOT / V4_POLICY_REL).read_bytes()
            ),
            "authorization_ref": V4_AUTH_REL.name,
            "authorization_digest": bytes_sha256(
                (ROOT / V4_AUTH_REL).read_bytes()
            ),
            "attempt_receipt_ref": V4_ATTEMPT_RECEIPT_REL.name,
            "attempt_receipt_digest": bytes_sha256(attempt_path.read_bytes()),
            "final_classification": attempt["final_classification"],
            "termination_reason": attempt["termination_reason"],
            "physical_provider_calls_v4": attempt[
                "physical_provider_calls"
            ],
            "retry_count_v4": attempt["retry_count"],
            "response_contract_equivalence": manifest[
                "response_contract_equivalence"
            ],
            "physical_attempt_classifications": [
                {
        ...
        _write_once(ROOT / V4_BLOCKED_REL, blocked)
        print(
            json.dumps(
                {
                    "blocked_receipt_sha256": bytes_sha256(
                        (ROOT / V4_BLOCKED_REL).read_bytes()
                    ),
                    "physical_provider_calls_v4": attempt[
                        "physical_provider_calls"
                    ],
    ...
    verification_path = ROOT / V4_VERIFICATION_REL
    if not verification_path.is_file():
        print(
            json.dumps(
                {"status": "PASS_AWAITING_LOCAL_VERIFICATION"},
    ...
    for field, expected in required.items():
        if verification.get(field) != expected:
            raise SystemExit(f"V4 verification does not prove {field}")
    ready = {
        "schema": V4_READY_SCHEMA,
        ...
        "attempt_receipt_ref": V4_ATTEMPT_RECEIPT_REL.name,
        "attempt_receipt_digest": bytes_sha256(attempt_path.read_bytes()),
        "verification_receipt_ref": V4_VERIFICATION_REL.name,
        "verification_receipt_digest": bytes_sha256(
            verification_path.read_bytes()
        ),
        "model": "gpt-5.4",
        ...
        "physical_provider_calls_v4": attempt["physical_provider_calls"],
        "retry_count_v4": attempt["retry_count"],
        "r1_worker_launch_authorized": True,
        "r2_launch_authorized": False,
        "training_started": False,
    ...
    _write_once(ROOT / V4_READY_REL, ready)
    print(
        json.dumps(
            {
                "ready_receipt_sha256": bytes_sha256(
                    (ROOT / V4_READY_REL).read_bytes()
                ),
                "status": "R1_PREFREEZE_READY",
            },
            sort_keys=True,
...
def main() -> int:
    ...
    if args.action == "dry-run-v3":
        return _dry_run_v3()
    if args.action == "finalize-v3":
        return _finalize_v3()
    if args.action == "prepare-v4":
        return _prepare_v4()
    if args.action == "dry-run-v4":
        return _dry_run_v4()
    if args.action == "record-v4-verification":
        return _record_v4_verification(
