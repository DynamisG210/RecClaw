#!/usr/bin/env python3
"""Build and verify the complete pre-outcome V22 gpu35 Pilot closure."""
...
RUNTIME_AUDIT = DOCS / "V22_GPU35_RUNTIME_AUDIT.json"
BACKEND_AUDIT = (
    DOCS / "V22_GPU35_BACKEND_CONFORMANCE_AUDIT.json"
)
SCIENTIFIC_GATE = DOCS / "V22_SCIENTIFIC_ATTRIBUTION_GATE.json"
DEFAULT_OUTPUT = DOCS / "V22_FROZEN_CHAIN_PILOT_CONTRACT.json"
DEFAULT_OUTPUT_ROOT = BACKEND_ROOT.parent / "pilot_9224_v22"
...
GIT_CONFIG_HOME = Path(
    "/NAS2020/Workspaces/DMGroup/tingrangan/"
    "recclaw_gpu35_backend_v2/tools/git-config"
)
...
def activate_original_git_tool() -> None:
    ...
    os.environ["XDG_CONFIG_HOME"] = GIT_CONFIG_HOME.as_posix()
...
def _canary_records() -> list[dict[str, Any]]:
    parent = BACKEND_ROOT / "backend_canaries_v22"
    records = []
...
def _build_resource_envelope(
    ...
    envelope = {
        ...
        "record_schema": "recclaw.v22-resource-envelope.v1",
...
def _source_projection(
    ...
    for relative, digest in m6i_projection.items():
        if relative not in post_m6i_runtime_files:
            if file_sha256(ROOT / relative) != digest:
                raise RuntimeError(
                    f"M6I qualified source changed before V22 freeze: {relative}"
                )
    ...
    overlay = {
        relative: file_sha256(ROOT / relative)
        for relative in V22_OVERLAY_FILES
    }
    projection = {
        "allowed_v16_byte_difference": {
            ...
            "V22_EXPLICIT_RESOURCE_INJECTION": (
        ...
        "record_schema": (
            "recclaw.v22-v16-scientific-source-projection.v1"
        ),
...
def _runtime_audit(
    ...
    audit = {
        ...
        "evidence_class": "DEVELOPMENT_ONLY_PRE_OUTCOME",
        "formal_acceptance": False,
        "record_schema": "recclaw.v22-gpu35-runtime-audit.v1",
...
def _backend_audit(
    ...
    audit = {
        ...
        "record_schema": (
            "recclaw.v22-gpu35-backend-conformance-audit.v1"
        ),
...
def _scientific_gate(
    ...
    gate = {
        ...
        "pilot_outcomes_used": False,
        "provider_calls": 0,
        "record_schema": "recclaw.v22-scientific-attribution-gate.v1",
        "runtime_audit_digest": runtime["runtime_audit_digest"],
        "scientific_source_projection_digest": source_projection[
            "projection_digest"
        ],
...
def _build_contract(
    *,
    ...
    store = V22PilotStoreContractV1.create()
    if (
        tuple(item.to_dict() for item in v22_arm_policies())
        != tuple(item.to_dict() for item in v16_arm_policies())
        or v22_resource_ceilings().to_dict()
        != envelope["resource_ceilings"]
    ):
        raise RuntimeError("V22 treatment/resource binding mismatch")
    assignment = PrivateTreatmentAssignmentV1.create(
        store.experiment_id,
        nonce=V22_PILOT_ASSIGNMENT_NONCE,
    )
    ...
    payload["original"] = {
        **v16["original"],
        "git_config_path": GIT_CONFIG.as_posix(),
        "git_config_sha256": file_sha256(GIT_CONFIG),
    ...
    payload["backend_migration"] = {
        "from_backend": "gpu5 RTX 4090 Runtime V9",
...
def verify_v22_pilot_contract(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or contract["pilot"]["search_seeds"] != [9224]
        ...
        raise RuntimeError("V22 contract closure invariant mismatch")
    for relative, digest in contract["source"]["files"].items():
        if file_sha256(ROOT / relative) != digest:
            raise RuntimeError(f"V22 source identity mismatch: {relative}")
    exact = {
    ...
    for artifact, digest in exact.items():
        if file_sha256(artifact) != digest:
            raise RuntimeError(f"V22 release identity mismatch: {artifact}")
    if _git_head() != contract["source"]["git_head_at_freeze"]:
        raise RuntimeError("V22 source repository head is not frozen source")
    subprocess.run(
        [
            str(GIT_EXECUTABLE),
            f"--git-dir={ROOT / '.git'}",
...
def build(output: Path, output_root: Path) -> dict[str, Any]:
    if output_root.exists():
        raise RuntimeError(f"fresh V22 output root already exists: {output_root}")
    if output.exists():
        raise RuntimeError(f"V22 contract already exists: {output}")
    ...
    verify_v22_pilot_contract(output)
    return contract
...
def main() -> int:
    ...
    if args.verify:
        contract = verify_v22_pilot_contract(args.output.resolve())
    else:
        contract = build(
            args.output.resolve(),
