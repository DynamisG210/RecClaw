...
def normalize_base_model(raw: str | None, override: str | None = None) -> str:
    ...
    if "custom" in text:
        return "Custom"
    # For unknown base models (e.g., new architecture families), preserve the
    # original name so that the runner can attempt a generic fallback.
    fallback = str(override or raw or "").strip()
    if fallback:
        return fallback
...
def build_plan(
    ...
    base_config_name = BASE_CONFIGS.get(base_model)
    config_files = [
        CONFIG_DIR / "task_ml1m.yaml",
    ]
    if base_config_name:
        config_files.append(CONFIG_DIR / base_config_name)
    config_files.append(CONFIG_DIR / "lightgcn_metrics.yaml")
...
def main() -> int:
    ...
    parser.add_argument("--base-model", choices=sorted(BASE_CONFIGS), help="Override ambiguous base_model (use Custom for new architecture families)")
