...
EXECUTION_PURPOSES = {"ordinary", "smoke", "confirmation", "baseline"}


class CandidateExecutionBudgetExhausted(RuntimeError):
    pass


def reserve_candidate_execution(
    *, candidate_id: str, run_id: str, purpose: str
) -> dict[str, Any] | None:
    raw_limit = os.environ.get("RECCLAW_CANDIDATE_EXECUTION_BUDGET", "").strip()
    raw_ledger = os.environ.get("RECCLAW_CANDIDATE_EXECUTION_LEDGER", "").strip()
    if not raw_limit and not raw_ledger:
        return None
    if not raw_limit or not raw_ledger:
        raise ValueError("candidate execution budget and ledger must be configured together")
    limit = int(raw_limit)
    if limit < 1:
        raise ValueError("candidate execution budget must be positive")
    ledger = Path(raw_ledger).expanduser().resolve()
    if not ledger.is_absolute():
        raise ValueError("candidate execution ledger must be absolute")
    try:
        ledger.relative_to(PROJECT_ROOT)
    except ValueError:
        pass
    else:
        raise ValueError("candidate execution ledger must be outside the source tree")
    ledger.parent.mkdir(parents=True, exist_ok=True)
    with ledger.open("a+", encoding="utf-8") as handle:
        fcntl.flock(handle.fileno(), fcntl.LOCK_EX)
        handle.seek(0)
        reservations = [line for line in handle if line.strip()]
        if len(reservations) >= limit:
            raise CandidateExecutionBudgetExhausted(
                f"candidate execution budget exhausted: {len(reservations)}/{limit}"
            )
        record = {
            "record_type": "RECCLAW_AB002_CANDIDATE_EXECUTION_RESERVATION",
            "sequence_index": len(reservations),
...
def main() -> int:
    ...
    parser.add_argument(
        "--execution-purpose",
        choices=sorted(EXECUTION_PURPOSES),
        default="ordinary",
        help="Audit label for the shared AB-002 candidate-execution budget.",
    )
    ...
    exit_code = 0
    try:
        reservation = reserve_candidate_execution(
            candidate_id=args.candidate_id,
            run_id=plan["run_id"],
            purpose=args.execution_purpose,
        )
        if reservation is not None:
            print(json.dumps({"candidate_execution_reservation": reservation}, sort_keys=True))
