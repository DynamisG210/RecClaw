...
def plan_payload(
    ...
    return {
        ...
        "max_implement_per_round": max_implement_per_round,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        choices=(1, 2),
        default=2,
        help="Maximum common candidate implementation attempts per scheduling round",
    )
    parser.add_argument(
        "--anchor-families",
        default=(
    ...
    parser.add_argument(
        "--experiment-directive",
        default=DEFAULT_EXPERIMENT_DIRECTIVE,
        help="Fixed experiment steering text passed identically to both arms.",
    )
    ...
    commands = build_commands(
        ...
        max_implement_per_round=int(args.max_implement_per_round),
        anchor_families=anchor_families,
        allow_llm_fallback=bool(args.allow_llm_fallback),
        experiment_directive=str(args.experiment_directive),
    )
    ...
    payload = plan_payload(
        ...
        int(args.max_implement_per_round),
        anchor_families,
    )
    if args.dry_run:
