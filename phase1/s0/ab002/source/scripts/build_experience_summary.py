...
def detect_plateau(
    ...
    *,
    ...
    weak_family_ceiling = max(0.0, float(plateau_policy.get("weak_family_ceiling", 0.2671)))
    trial_rows = [
        row
        for row in rows
...
def summarize_memory(
    ...
    *,
    ...
    anchor_floor = float(algorithm_policy.get("anchor_floor", baseline_reference.get("LightGCN", 0.2671)))
    ...
    for family, rows in sorted(family_rows.items()):
        ...
        is_high_potential = (
            ...
            or (best is not None and best >= anchor_floor)
        )
        if not values:
            policy_bucket = "avoid"
        ...
        family_summaries.append(
            {
                ...
                "predeclared_anchor": family in anchor_families,
